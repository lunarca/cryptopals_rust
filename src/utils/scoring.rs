...
pub fn score_english_plaintext(plaintext: String) -> i32 {
  let english_frequency: Vec<char> = vec![
    ...
    "<", ">", "2", "3", "0", "4", "5", "_", "*", "6", "7", "8", "|", "&", "@", "/", "}", "#",
    "=", "%", "~", "`"
  ].iter().map(|charstring| charstring.chars().next().unwrap()).collect();
...
fn calculate_distance(chr: char, base_frequency_order: &Vec<char>, index: usize) -> i32 {
...
pub fn calculate_frequencies(plaintext: String) -> HashMap<char, i32> {
  let results: HashMap<char, i32> = HashMap::new();

  let frequencies = plaintext.chars().fold(results, |acc, ch| {
